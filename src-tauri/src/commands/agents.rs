...
pub async fn fetch_github_agents() -> Result<Vec<GitHubAgentFile>, String> {
    ...
    let url = "https://api.github.com/repos/rixmerz/nova/contents/cc_agents";

    let response = client
        .get(url)
        .header("Accept", "application/vnd.github+json")
        .header("User-Agent", "Nova-App")
        .send()
        .await
    ...
    // Filter only .nova.json agent files
    let agent_files: Vec<GitHubAgentFile> = api_files
        .into_iter()
        .filter(|f| f.name.ends_with(".nova.json") && f.file_type == "file")
        .filter_map(|f| {
