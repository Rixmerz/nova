...
interface SessionState {
  ...
  deleteSessions: (sessionIds: string[], projectId: string) => Promise<void>;
  clearError: () => void;
...
> = (set, get) => ({
  ...
  deleteSession: async (sessionId: string, projectId: string) => {
    try {
      // Call the API to delete the session from disk
  ...
  deleteSessions: async (sessionIds: string[], projectId: string) => {
    try {
      await api.deleteSessions(sessionIds, projectId);
      ...
      set((state) => {
        ...
        return {
          sessions: {
            ...state.sessions,
            [projectId]: state.sessions[projectId]?.filter((s) => !sessionIdsSet.has(s.id)) || []
          },
          currentSessionId: state.currentSessionId && sessionIdsSet.has(state.currentSessionId) ? null : state.currentSessionId,
          currentSession: state.currentSession && sessionIdsSet.has(state.currentSession.id) ? null : state.currentSession,
          sessionOutputs: Object.fromEntries(
            Object.entries(state.sessionOutputs).filter(([id]) => !sessionIdsSet.has(id))
          )
        };
      });
    } catch (error) {
